...
import utils
from models.int_tqu_llama_layer import TQULlamaDecoderLayer
...
MB = 1024.0 * 1024.0
def update_sublayers_lwc_params(sub_layers, updated_lwc_params):
    for n, m in sub_layers.named_parameters():
        if n.find('bound_factor') > -1:
            m.data.copy_(next(updated_lwc_params).data)
...
def tqu(
    ...
    if "llama" in args.model or "Llama" in args.model:
        ...
        DecoderLayer = TQULlamaDecoderLayer
    else:
    ...
    if args.resume_reassembly:
        tqu_cr_parameters = torch.load(args.resume_reassembly)
    else:
        tqu_cr_parameters = {}


    if args.resume_lora:
        tqu_lora_parameters = torch.load(args.resume_lora)
    else:
        tqu_lora_parameters = {}
    ...
    for r in range(num_round):
        ...
        if args.resume_reassembly and r in tqu_cr_parameters:
            sub_layers_dict = tqu_cr_parameters[r]
            ...
            tqu_cr_parameters[r] = sub_layers_dict
            torch.save(
                tqu_cr_parameters,
                os.path.join(args.output_dir, f"tqu_cr_parameters.pth"),
            )
        ...
        (lwc_params,lwc_params_names) = get_lwc_parameters(sub_layers, r)
        # list(lwc_parameters(sub_layers))
        ...
        if args.resume_lora and r in tqu_lora_parameters:
            sub_layers_dict = tqu_lora_parameters[r]
            ...
            # updated_lwc_params = list(lwc_parameters(sub_layers))
            # update_sublayers_lwc_params(sub_layers, updated_lwc_params)
            logger.info("Updated sub_layers with new LWC parameters")
            del optimizer
            gc.collect()
            torch.cuda.empty_cache()
            ...
            tqu_lora_parameters[r] = sub_layers_dict
            torch.save(
                tqu_lora_parameters,
                os.path.join(args.output_dir, f"tqu_lora_parameters.pth"),
            )
