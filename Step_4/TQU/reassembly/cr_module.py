...
class CRModule(nn.Module):
    ...
    self, x, x_max, attention_mask, position_ids, fcs, function="fc1"
        ...
        x_max_float32 = x_max.to(torch.float32)  # Convert to float32
        Q3 = torch.quantile(x_max_float32, 0.75).item()  # Perform quantile operation
        Q1 = torch.quantile(x_max_float32, 0.25).item()  # Perform quantile operation
        Q3 = torch.tensor(Q3, dtype=torch.bfloat16)  # Convert back to bfloat16 if necessary
        Q1 = torch.tensor(Q1, dtype=torch.bfloat16)  # Convert back to bfloat16 if necessary
        IQR = Q3 - Q1
    
        # Define best_th as Q3 + 1.5 * IQR
        best_th = Q3 + 1.5 * IQR
        return best_th
    # def find_threshold(
    #     self, x, x_max, attention_mask, position_ids, fcs, function="fc1"
