...
class TQULlamaDecoderLayer(nn.Module):
    ...
    def tqu_lora_state_dict(self, destination=None, prefix="", keep_vars=False):
    ...
    def tqu_sm_state_dict(self, destination=None, prefix="", keep_vars=False):
        if destination is None:
            destination = OrderedDict()
    ...
    def tqu_state_dict(self, destination=None, prefix="", keep_vars=False):
        if destination is None:
            destination = OrderedDict()
        destination = self.tqu_lora_state_dict(destination, prefix, keep_vars)
        destination = self.tqu_sm_state_dict(destination, prefix, keep_vars)
        return destination
