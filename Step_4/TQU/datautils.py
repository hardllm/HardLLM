...
def get_dial(nsamples, seed, seqlen, model):
    print("get_dial")
    print("load data from network")
    traindata = load_dataset("knkarthick/dialogsum", split="train")
    testdata = load_dataset("knkarthick/dialogsum", split="val")

    tokenizer = AutoTokenizer.from_pretrained(model, use_fast=False)
    train_texts = [f"An AI tool that summarizes users' dialogue.\n### Input: ```{sentence}```\n### Output: {summary}"
               for sentence, summary in zip(traindata["dialogue"], traindata["summary"])]

    test_texts = [f"An AI tool that summarizes users' dialogue.\n### Input: ```{sentence}```\n### Output: {summary}"
              for sentence, summary in zip(testdata["dialogue"], testdata["summary"])]
...
def get_loaders(name, nsamples=128, seed=0, seqlen=2048, model=""):
    if "dial" in name:
        return get_dial(nsamples, seed, seqlen, model)
