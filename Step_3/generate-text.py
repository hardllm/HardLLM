...
def process_json(input_training_file):
    data = []
    with open(input_training_file, 'r', encoding='utf-8') as rf:
        for line in rf:
            data.append(json.loads(line.strip()))
        
        # Assumption: data is a list of dictionaries
        prompt_counter = defaultdict(int)

        for entry in data:
            # Find all keys containing "label" in their name
            label_columns = [key for key in entry.keys() if "label" in key]
            # Create the prompt from the label columns
            prompt = "\t".join([str(entry[label]) for label in label_columns]) + "\n\n"
...
def convert_model(checkpoint_path):
    ...
    sd = load_file(os.path.join(checkpoint_path, "model.safetensors"))
    state_dict = {}
    for key, value in sd.items():
        key = key.replace("_module.module.", "")
        state_dict[key] = value
...
def main():
    ...
    with torch.no_grad():
        ...
        prompt_counter = process_json(args.input_training_file)
    ...
    # output_path = os.path.join(args.output_dir, str(args.length) + ".generations.csv")
    ...
    os.makedirs(args.output_dir, exist_ok=True)
    output_path = os.path.join(args.output_dir, str(args.length) + ".generations.jsonl")
