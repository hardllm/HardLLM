...
def main(args: Arguments):
    ...
    # data_path_train = os.path.join(args.model.data_dir, "selected_no_noise.jsonl")
    # clean_data_path_train = os.path.join(args.model.data_dir, "selected_no_noise_clean.jsonl")
    
    # # Load train dataset
    # train_dataset = datasets.load_dataset('json', data_files={'train': data_path_train})['train']
    
    # # Remove columns
    # train_dataset = train_dataset.remove_columns(["text_id", "probability"])
    
    # # Save the cleaned dataset to a new file
    # train_dataset.to_json(clean_data_path_train)
    data_path_train = os.path.join(args.model.data_dir, "selected_no_noise_clean.jsonl")
    data_path_val = os.path.join(args.model.data_dir, "validation.jsonl")
    dataset = datasets.load_dataset('json', data_files={'train': data_path_train, 'validation': data_path_val})
    ...
    def preprocess_function(examples):
        ...
        for t in range(len(examples['text'])):
            text = "\t".join([str(examples[name][t]) for name in label_column_names]) + "\n\n" + examples['text'][t] + tokenizer.eos_token
            batch.append(text)
